...
int dataSet2::RNG(vector<int> validNumbers, int upperPound, int lowerPound, int a, int b, int c){
    ...
    int sum = a+b+c;
    discrete_distribution<int> distribution{(static_cast<double>(a))/sum, (static_cast<double>(b))/sum,  (static_cast<double>(c))/sum};
...
void dataSet2::initialization(string fileName){
    ...
    for(int i=0; i<Node::getTotalNum(); i++){
        Node newNode;
        file << newNode.getName() << "," << to_string(newNode.position.getXaxis()) << "," << to_string(newNode.position.getYaxis()) << "," << to_string(newNode.position.getZaxis())
